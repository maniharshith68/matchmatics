...
export const FormNba: React.FC = () => {
  ...
  useEffect(() => {
  fetch('http://localhost:4000/api/nba/options')
    .then(res => res.json())
    .then(setOptions)
    .catch(console.error)
  ...
  try {
    const res = await fetch('http://localhost:4000/api/nba/query', {
      method: 'POST',
          ...
          <Card.Body>
            <Form onSubmit={handleSubmit}>
              <Row>
                <Col md={6} className="mb-3">
                  <Form.Group>
                    ...
                    onChange={e => handleChange('season_type', e.target.value)}
                  >
              ...
              <Row>
                <Col md={6} className="mb-3">
                  <Form.Group>
                    ...
                    <Form.Select
                    className="bg-dark text-light border-info"
                    value={formData.team_name_home}
                    onChange={e => handleChange('team_name_home', e.target.value)}
                  >
                    <option value="">Select Home Team Name</option>
                ...
                <Col md={6} className="mb-3">
                  <Form.Group>
                    <Form.Label>Win/Loss at Home</Form.Label>
                    <Form.Select
                    className="bg-dark text-light border-info"
                    value={formData.wl_home}
                    onChange={e => handleChange('wl_home', e.target.value)}
                  >
                    <option value=""> Win/Loss at Home</option>
              ...
              <Row>
                <Col md={6} className="mb-3">
                  <Form.Group>
                    <Form.Label>Matchup at Home</Form.Label>
                    <Form.Select
                    className="bg-dark text-light border-info"
                    value={formData.matchup_home}
                    onChange={e => handleChange('matchup_home', e.target.value)}
                  >
                    <option value="">Select Matchup at Home</option>
